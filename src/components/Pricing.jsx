...
const Card = ({ children, className="" }) => <div className={`card p-6 ${className}`}>{children}</div>
...
export default function Pricing(){
  ...
  return (
    <section id="pricing" className="bg-slate-900 py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <h2 className="text-center text-3xl md:text-4xl font-bold">Planos & Go-to-Market</h2>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {tiers.map(t=>(
            <Card key={t.name} className={t.featured ? 'ring-2 ring-cyan-400/60' : ''}>
              ...
              <ul className="mt-4 grid gap-2 text-slate-300 list-disc pl-5">{t.bullets.map(b=><li key={b}>{b}</li>)}</ul>
