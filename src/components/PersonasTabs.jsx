  ...
  return (
    <section className="bg-slate-900 py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <h2 className="text-center text-3xl md:text-4xl font-bold">Para quem a VYRA existe</h2>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          {personas.map((x,idx)=>(
            <button key={x.name} onClick={()=>setI(idx)} className={`px-4 py-2 rounded-xl text-sm ${i===idx?'btn-primary':'btn'}`}>{x.name}</button>
          ))}
        </div>
        <Card className="mx-auto mt-8 max-w-5xl">
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              ...
              <ul className="mt-2 grid gap-2 list-disc pl-5 text-slate-300">
